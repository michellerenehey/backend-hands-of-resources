...
module.exports = class Housewife {
  ...
  static async insert({ name, season }) {
    const { rows } = await pool.query(
      'INSERT INTO housewives (name, season) VALUES ($1, $2) RETURNING *',
      [name, season]
    );
  ...
  static async findAll() {
    const { rows } = await pool.query('SELECT * FROM housewives');
    return rows.map((row) => new Housewife(row));
  }

  static async findById(id) {
    const { rows } = await pool.query('SELECT * FROM housewives WHERE id=$1', [
      id,
    ]);
    if (!rows[0]) return null;
    return new Housewife(rows[0]);
  ...
  static async updateById(id, attributes) {
    ...
    if (!rows[0]) return null;
    // early return if there are no rows, prevents errors
    return new Housewife(rows[0]);
  }

  static async deleteById(id) {
    const { rows } = await pool.query(
      'DELETE FROM housewives WHERE id=$1 RETURNING *',
      [id]
    );
    if (!rows[0]) return null;
    return new Housewife(rows[0]);
